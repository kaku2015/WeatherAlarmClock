	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
		...
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			...
			boolean vibrate = cursor.getInt(cursor
					.getColumnIndex(WeacDBMetaData.AC_VIBRATE)) == 1 ? true
					: false;
			boolean nap = cursor.getInt(cursor
					.getColumnIndex(WeacDBMetaData.AC_NAP)) == 1 ? true : false;
			...
			boolean weaPrompt = cursor.getInt(cursor
					.getColumnIndex(WeacDBMetaData.AC_WEA_PROMPT)) == 1 ? true
					: false;
			boolean onOff = cursor.getInt(cursor
					.getColumnIndex(WeacDBMetaData.AC_ON_OFF)) == 1 ? true
					: false;
